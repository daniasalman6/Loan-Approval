...
All three models are highly significant to affect on loan approval, as they all have p-values less than the standard level of statistical significance of .05. Model 1 uses CIBIL Score only.M1 was the best performing given the data set so it is not worth adding additional features, we used log reg because the final result (loan approval) is a binary categorical variable, EDA showed cibil score and loan term had strongest relationship with approval so we used those in the log reg models. To answer the question that is posed, we reject the null hypothesis and state that Model1 with just the CIBIL score is the best predictor.
