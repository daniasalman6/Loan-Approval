...
After doing Chi-squared testing on all three variables (cibil credit score, annual income, and loan amount), cibil credit score was the only variable with a low p-value meaning we reject the null hypothesis that cibil score and loan approval are independent (have no relationship). We are forced to accept the null hypotheses that annual income and loan apporval are independent and that loan amount and loan approval are independent.
...
First let us run Chi-squared tests to determine dependance of variables.
...
The third model that incorporated all three predictors (cibil credit score, annual income, and loan amount) had the highest R-squared value of 0.63 compared to the other two models' value of 0.623. The third model also had an accuracy of 92.1%, a slight increase from the first and second model's accuracy of 91.8%. Additionally, because because annual income amount was added as a feature to the second model and did not see a change throughout different model evaluation steps, it can be stated that income_annum is not a significant predictor.
...
Now we will conduct logistical regression with three different models and evaluate them to see which is most effective and precise.
