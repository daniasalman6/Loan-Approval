...
Contingency Table
The raw counts show a very similar distribution of approvals and rejections across both groups.Graduate applicants had 1,329 approvals and 728 rejections.Not Graduate applicants had 1,311 approvals and 803 rejections.This similarity suggests the approval rates are nearly identical: ~62.4% for graduate and 62% for non-graduate.

Chi-Square Test
The test statistic is .083958. This indicates that the data is close to what you would expect.
The p-value is .772. It is much larger than the standard significance level of .05, we fail to reject the null hypothesis. This is the formal statistical confirmation that there is no significant association between an applicant's education level and their loan status.
...
```
The Wilcoxon Rank sum tests is utilized to compared the different asset types between approved and rejected loan applicants. Across the four asset categories, the p-values were greater than .05, indicating that there are no statistically significant differences in asset distributions between loan approval statuses.
```{r Q2 Part 2}
